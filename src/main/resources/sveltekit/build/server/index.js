import { c as create_ssr_component, s as setContext, v as validate_component, m as missing_component } from './chunks/ssr-DhdZ8Nh2.js';
...
import { w as writable, r as readable } from './chunks/index-t0gNv4Rz.js';
  ...
  templates: {
	...
	<body data-sveltekit-preload-data="hover">
		<style>
			...
			/* 상품 리스트 보이게하는 구문 */
			/* Media query for screen widths 800px or less */
			.grid-container {
				width: 100%;
				max-width: 1200px;
			...
			.grid-item {
				...
				flex-direction: column;
				align-items: center;
				border-radius: 5%; /* Makes the container rounded */
				border: 1px solid #ccc; /* Adds a border to the container */
				overflow: hidden;
				background: #fff; /* Adds a background color to the container */
				box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); /* Adds a subtle shadow */
			}
			.grid-item img {
				width: 100%;
				height: calc(10vw);
				object-fit: fill; /* Ensures the image covers the container while maintaining aspect ratio */
				border-bottom: 1px solid #ccc; /* Adds a border below the image */
			}

			.product-name,
			.product-price {
				text-align: center;
				background-color: #fff; /* Optional: Adds a background color to the text */
			}
			/* 정보 수정 모달 */
			.input-group {
				display: flex;
			...
			.nav-brand .brand-text {
				font-size: 2rem; /* 텍스트 크기 조절 */
				font-weight: bold;
			}
			.nav-brand .logo {
				height: 3rem; /* 로고 이미지 높이 조절 */
				width: auto; /* 가로 비율 자동 조절 */
				margin-right: 1rem; /* 텍스트와의 간격 조절 */
			}
			/* mainview */
			.gallery {
			...
			/* 모달 테두리 색상 */
			.custom-modal {
				border: 8px solid rgba(252, 186, 3, 0.8);
			}

			/* 모달 버튼 색상 */
			.modal_button {
				background-color: #ea9e2f;
			}
			/* detail view */
			.element {
				margin-top: 25px;
			...
			.main-image {
				width: 100%;
				/* height: 100%; */
				object-fit: contain; /* 이미지가 컨테이너에 맞게 조절됨 */
			}
			.thumbnail-image {
				width: 100px;
				height: 100px;
